...
export default graph => {
        ...
        if (graph.container != null && !graph.transparentBackground) {
            if (graph.pageVisible) {
                ...
                if (graph.view.backgroundPageShape == null) {
                    ...
                    if (firstChild != null) {
                        graph.view.backgroundPageShape = graph.view.createBackgroundPageShape(
                            bounds
                        );
    ...
    graph.panGraph = function(dx, dy) {
        ...
        if (graph.view.shiftPreview1 != null) {
            ...
            var position = -Math.round(
                    phase -
                    mxUtils.mod(graph.view.translate.x * graph.view.scale + dx, phase)
                ) +
                "px " +
                -Math.round(
                    phase -
                    mxUtils.mod(graph.view.translate.y * graph.view.scale + dy, phase)
                ) +
                "px";
            canvas.style.backgroundPosition = position;
        }
    ...
    graph.updatePageBreaks = function(visible, width, height) {
        ...
        visible =
            visible && Math.min(bounds.width, bounds.height) > graph.minPageBreakDist;
        ...
        var drawPageBreaks = mxUtils.bind(graph, function(breaks) {
            if (breaks != null) {
                var count =
                    breaks == graph.horizontalPageBreaks ?
                    horizontalCount :
                    verticalCount;
    ...
    var mxGraphHandlerShouldRemoveCellsFromParent =
        graphHandler.shouldRemoveCellsFromParent;
    ...
    var mxConnectionHandlerCreateMarker = connectionHandler.createMarker;
    connectionHandler.createMarker = function() {
    ...
    var graphHandlerGetInitialCellForEvent = graphHandler.getInitialCellForEvent;
    graphHandler.getInitialCellForEvent = function(me) {
        ...
        if (psel == null || (psel != cell && psel != parent)) {
            while (!graph.isCellSelected(cell) &&
                !graph.isCellSelected(parent) &&
                model.isVertex(parent) &&
                !graph.isContainer(parent)
            ) {
                cell = parent;
    ...
    var graphHandlerIsDelayedSelection = graphHandler.isDelayedSelection;
    ...
    graphHandler.selectDelayed = function(me) {
        if (!graph.popupMenuHandler.isPopupTrigger(me)) {
            var cell = me.getCell();
            ...
            if (state != null && me.isSource(state.control)) {
                ...
                while (!graph.isCellSelected(parent) && model.isVertex(parent)) {
                    cell = parent;
